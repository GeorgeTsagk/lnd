...
func (lc *LightningChannel) signLocalHtlcAuxSigs() ([]byte, error) {
	...
	for i, sigOpt := range auxSigs {
		...
		// The natural path keeps Incoming equal to the original HTLC
		// direction used for aux-output lookup. The alternate path flips
		// Incoming for script generation but preserves
		// IncomingHTLCLookup.
