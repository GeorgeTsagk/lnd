...
func (lc *LightningChannel) signLocalHtlcAuxSigs() ([]byte, error) {
	...
	// Collect the signatures from the batch. Sigs come in pairs
	// (primary + alt) per HTLC, sorted by output index.
	...
	// Pack sigs into a blob tagged by HTLC index. The batch has 2
	// entries per HTLC (primary at even index, alt at odd), sorted
	// by output index.
	var entries []revocationAuxSigEntry
	for i := 0; i+1 < len(auxSigs); i += 2 {
		htlcIdx := auxSigBatch[i].HTLC.HtlcIndex
		entries = append(entries, revocationAuxSigEntry{
			htlcIndex:  htlcIdx,
			primarySig: auxSigs[i].UnwrapOr(nil),
			altSig:     auxSigs[i+1].UnwrapOr(nil),
		})
	}
