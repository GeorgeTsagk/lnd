...
=======
		...
		if justiceCtx != nil {
			...
			// Preserve second-level awareness from the justice tx
			// context snapshot. The current breachedOutputs view can lag
			// after morphing/rebuilds, and if we lose this bit for a mixed
			// justice tx we stop looking up refreshed input proofs for the
			...
			for _, txIn := range spendingTx.TxIn {
				...
				wt := bo.WitnessType()
				//nolint:ll
				if wt == input.HtlcSecondLevelRevoke ||
...
func recordJusticeTxVariants(variants *justiceTxVariants,
	...
	record := func(jtx *justiceTxCtx) {
		...
		// The historic map is only used to match later confirmed spends
		// against the justice variants that were published at the time.
		// Store an immutable snapshot so later rebuilds can't mutate the
		// metadata associated with an older txid and retroactively change
		// whether that tx should be treated as carrying second-level inputs.
		snapshot.inputs = nil
